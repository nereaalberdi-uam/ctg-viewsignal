...
def plot_fhr_with_decelerations(fhr, fs, baseline, decelerations):
    ...
    plt.plot(time, fhr, label='Fetal Heart Rate (FHR)', color='blue')

    # Plot the baseline center and the window size
    plt.axhline(y=baseline.center, color='green', linestyle='--', label=f'Baseline Center ({baseline.center} BPM)')
    plt.fill_between(time, baseline.center - baseline.window_size / 2, baseline.center + baseline.window_size / 2, 
                     color='green', alpha=0.2, label=f'Baseline Window ({baseline.window_size} BPM)')

    # Highlight the deceleration areas with a shaded region
    for decel in decelerations:

        # Fill the area between the signal and the baseline center
        plt.fill_between(time[decel.start_idx:decel.end_idx], fhr[decel.start_idx:decel.end_idx], baseline.center,
                         color='red', alpha=0.3)

        # Plot the start, nadir and end of the deceleration
        plt.plot([decel.start_time, decel.nadir_time, decel.end_time], [baseline.center, decel.nadir_value, baseline.center], 'ro')

    # Set plot labels and title
    plt.xlabel('Time (s)')
    plt.ylabel('Fetal Heart Rate (BPM)')
    plt.title('Fetal Heart Rate (FHR) with Decelerations')
...
def get_classified_decelerations(fhr, uc, fs, verbose = False):
    decelerations, dBaseline = get_decelerations(fhr, fs, verbose = verbose)
    contractions = get_contractions(uc, fs, window_size = 3, verbose = verbose)
    if verbose:
